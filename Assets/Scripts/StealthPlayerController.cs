...
public class StealthPlayerController : Character
{
	...
	public float flyingEnergyMultiplier = 5.5f;
	...
	void Update()
	{
		...
		if (canCloak && !flying && Input.GetButtonDown("Cloak") && (state == States.idle || state == States.moving))
		{
		...
		if (canFly && !cloaked)
		{
			if (Input.GetButtonDown("Fly") && (state == States.idle || state == States.moving) && IsOnFloor())
			{
				Debug.Log("Fly");
		...
		if (state == States.idle && !cloaked && !flying && IsOnFloor())
		{
		...
		if (enableEnergyDrain)
		{
			...
			if (flying)
			{
				energyDrain = energyDrain * flyingEnergyMultiplier;
			}
